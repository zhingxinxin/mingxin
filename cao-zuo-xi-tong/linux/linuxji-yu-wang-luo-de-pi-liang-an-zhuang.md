...
5. 在TFTP服务安装完后，会创建一个/tftpboot目录，这个目录便是我们用来放置bootstrap引导程序\(pxelinux.0\)，bootstrap配置文件\(default\)，内核映像\(vmlinuz\)和文件系统文件\(initrd.img\)的。这里我们需要先创建一个pxelinux.cfg目录，并将default文件放在其下。
